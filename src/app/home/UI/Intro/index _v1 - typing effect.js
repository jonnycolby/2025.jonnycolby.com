...
class Intro extends React.Component {
    constructor(props) {
        ...
        Z.static = {
            // interval_min: 1000 * 0.05,
            // interval_max: 1000 * 0.32,
            interval_min: 1000 * 0.16,
            interval_max: 1000 * 0.16,
            visible_ms: 1000 * 1.6,
        ...
        Z.state = {
            ...
            max_duration: Z.static.interval_max * Z.static.text.length + Z.static.visible_ms,
        };
    }
    componentDidMount() {
        const Z = this;
    ...
    start = async () => {
        const Z = this;
        //
        await pause(1000 / 15); // make sure there's at least one frame between DOM existence and animation start
        await Z.set_state({ spacing_active: true });
        //
        for (let char_i = 0; char_i < Z.static.text.length; char_i++) {
            await pause(Z.static.interval_min + Math.random() * (Z.static.interval_max - Z.static.interval_min));
            await Z.set_state({ characters_visible: char_i + 1 });
        }
        await pause(Z.static.visible_ms);
            ...
            >
                <div className={`${styles.intro_text} ${Z.state.visible ? styles._visible : ""}`}>
                    <div
                        ...
                        style={{
                            transition: `letter-spacing ${Z.state.max_duration * 0.001}s linear`,
                        }}
                    >
